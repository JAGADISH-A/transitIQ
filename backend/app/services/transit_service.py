...
class TransitService:
    ...
    def find_two_transfer_routes(self, source_stop_id: str, destination_stop_id: str, departure_after: str | None = None) -> list["TransferJourney"]:
        ...
        DEBUG_TRANSFER_SEARCH = False
        
        if DEBUG_TRANSFER_SEARCH:
            self.logger.info("\n[2_TRANSFER_SEARCH]\n%s\n%s", source_stop_id, destination_stop_id)
        ...
        for feed_name, loader in self._feeds.items():
            ...
            # Pre-sort leg2_valid by departure_time_A to consider earlier trips first
            if "departure_time_A" in leg2_valid.columns:
                leg2_valid = leg2_valid.sort_values(by="departure_time_A")

            # Filter out second leg departures that occur before the first leg could possibly arrive at any transfer stop
            ...
            # Pre-build lookup dictionaries (O(1) lookups)
            ...
            # Map stop_id to list of dictionaries for O(1) group lookups
            arrivals_by_stop = {}
            ...
            for t_a_id in s_a:
                ...
                for leg1_arr_row in t_a_arrs:
                    ...
                    try:
                        # Fast splitting instead of parsing regex
                    ...
                    for leg2_row in leg2_options:
                        ...
                        # Find valid leg3 departures at B
                        valid_leg3_deps = []
                        for dep in t_b_deps:
                        ...
                        for leg3_dep_row, wait2, d3_total in valid_leg3_deps:
                            ...
                            if evaluated_candidates > MAX_EVALUATED_CANDIDATES:
                                self.logger.warning("[TWO_TRANSFER_CAP] Candidate evaluation cap reached (%d)", MAX_EVALUATED_CANDIDATES)
                                cap_reached = True
                            ...
                            if time.monotonic() - search_start > SEARCH_TIMEOUT_SECONDS:
                                self.logger.warning("[TWO_TRANSFER_TIMEOUT] Search exceeded %ds", SEARCH_TIMEOUT_SECONDS)
                            ...
                            # No early-exit check to evaluate all candidates under the hard limits

        elapsed_seconds = time.monotonic() - search_start
        self.logger.info(
            f"[TWO_TRANSFER_PERFORMANCE]\n"
            f"candidates_evaluated={evaluated_candidates}\n"
            f"candidates_accepted={len(transfer_journeys)}\n"
            f"elapsed_seconds={elapsed_seconds:.4f}"
        )

        # Log search statistics once at the end
        self.logger.info(
            "[TWO_TRANSFER_SEARCH_COMPLETE] evaluated=%d, found=%d, cap_reached=%s, timeout=%s, duration=%.2fs",
            evaluated_candidates,
            len(transfer_journeys),
            cap_reached,
            timeout_reached,
            elapsed_seconds
        )
